...
public:
	...
	**/
	...
	/**
	* Resets this Matrix to a rotation.
	* @param x Rotation direction along x-axis.
	* @param y Rotation direction along y-axis.
	* @param z Rotation direction along z-axis.
	* @param r The angle in radians.
	**/
	...
	**/
	void setScale(float sx, float sy, float z);
	...
	**/
	...
	/**
	* Creates a new 3D look-at matrix.
	**/
...
void Matrix4::transformXYZ(Vdst *dst, const Vsrc *src, int size) const
{
	for (int i = 0; i < size; i++)
	{
		...
		float w = (e[3]*src[i].x) + (e[7]*src[i].y) + (e[11]*src[i].z) + (e[15]);
		w = 1.0f / w;
