...
void Matrix4::setRotation(float x, float y, float z, float rad)
{
	float sine = std::sin(rad);
	float cosine = std::cos(rad);
	float theta = (1.0f - cosine);

	e[0] = (theta * x * x + cosine);
	e[4] = (theta * x * y - sine * z);
	e[8] = (theta * x * z + sine * y);

	e[1] = (theta * x * y + sine * z);
	e[5] = (theta * y * y + cosine);
	e[9] = (theta * y * z - sine * x);

	e[2] = (theta * x * z - sine * y);
	e[6] = (theta * y * z + sine * x);
	e[10] = (theta * z * z + cosine);
}
...
Matrix4 Matrix4::perspective(float fieldOfView, float aspectRatio, float near, float far)
{
	...
	float cotangent = (1.0f / std::tan(fieldOfView / 2.0f));
	float farNear = (1.0f / (far - near));

	m.e[0] = (cotangent / aspectRatio);
	m.e[5] = cotangent;
	m.e[10] = (-(far + near) * farNear);
	m.e[11] = -1.0f;
	m.e[14] = (-(2.0f * far * near) * farNear);
	m.e[15] = 0.0f;
