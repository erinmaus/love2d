...
void Graphics::rotate(float x, float y, float z, float angle)
{
	transformStack.back().rotate(x, y, z, angle);
}
...
{
	transformStack.back().scale(x, y, z);
	pixelScaleStack.back() *= (fabs(x) + fabs(y)) / 2.0;
}

void Graphics::translate(float x, float y, float z)
{
	transformStack.back().translate(x, y, z);
}
...
void Graphics::lookAt(float eyeX, float eyeY, float eyeZ, float targetX, float targetY, float targetZ, float upX, float upY, float upZ)
{
	Matrix4 lookAtMatrix = Matrix4::lookAt(eyeX, eyeY, eyeZ, targetX, targetY, targetZ, upX, upY, upZ);
	transformStack.back() *= lookAtMatrix;
}

void Graphics::setPerspective(float fieldOfView, float aspectRatio, float near, float far)
{
	projectionMatrix = Matrix4::perspective(fieldOfView, aspectRatio, near, far);
}
...
void Graphics::setOrtho(int w, int h, bool flipY)
{
	if (flipY)
	{
		projectionMatrix = Matrix4::ortho(0.0, (float)w, (float)h, 0.0f, -10.0f, 10.0f);
	}
	else
	{
		projectionMatrix = Matrix4::ortho(0.0f, (float)w, 0.0f, (float)h, -10.0f, 10.0f);
	}
}
