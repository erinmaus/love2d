...
int w_scale(lua_State *L)
{
	...
	float sz = (float)luaL_optnumber(L, 3, sx);
	instance()->scale(sx, sy, sz);
	return 0;
}
...
int w_lookAt(lua_State *L)
{
	float eyeX = (float)luaL_checknumber(L, 1);
	float eyeY = (float)luaL_checknumber(L, 2);
	float eyeZ = (float)luaL_checknumber(L, 3);
	float targetX = (float)luaL_checknumber(L, 4);
	float targetY = (float)luaL_checknumber(L, 5);
	float targetZ = (float)luaL_checknumber(L, 6);
	float positionX = (float)luaL_checknumber(L, 7);
	float positionY = (float)luaL_checknumber(L, 8);
	float positionZ = (float)luaL_checknumber(L, 9);
	instance()->lookAt(eyeX, eyeY, eyeZ, targetX, targetY, targetZ, positionX, positionY, positionZ);

	return 0;
}

int w_perspective(lua_State *L)
{
	...
	// far <= 0.0f is unnecessary since near > far, and near > 0.0f at this point

	instance()->setPerspective(fieldOfView, aspectRatio, near, far);
	return 0;
}

int w_project(lua_State *L)
{
	float x = (float)luaL_checknumber(L, 1);
	float y = (float)luaL_checknumber(L, 2);
	float z = (float)luaL_checknumber(L, 3);

	Vector3 worldSpacePoint(x, y, z);
	Vector3 result;
	instance()->project(worldSpacePoint, result);
...
int w_unproject(lua_State *L)
{
	...
	Vector3 worldSpacePoint(x, y, z);
	Vector3 result;
	instance()->unproject(worldSpacePoint, result);
